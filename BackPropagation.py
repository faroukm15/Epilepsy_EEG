...
class BackPropagation:
    def __init__(self, TrainingData=[], ValidationData=[], TestingData=[], ActivationFunction=0, StoppingCriteria=0, Layers=1, NeuronsPerLayer=[1], MSEThreshold = -1, Bias = False, eta=0.001, epochs=500, FeaturesMax=[], FeaturesMin=[]):
        ...
        if self.BiasUse == True:
            ...
            for i in range(len(self.TrainingData)):
                self.TrainingData[i].insert(0, 1)
            for i in range(len(self.TestingData)):
                self.TestingData[i].insert(0, 1)
            for i in range(len(self.ValidationData)):
                self.ValidationData[i].insert(0, 1)
        ...
        for k in range(self.Layers):
            if k == 0:  # Input Layer
                ...
                for h in range(len(self.Network.Layers[k].Neurons)):
                    self.Network.Layers[k].Neurons[h].Value = Sample[h]
                    self.Network.Layers[k].Neurons[h].input = Sample[h]
                continue
            for h in range(len(self.Network.Layers[k].Neurons)):
                V = 0.0
            ...
            for j in range(self.Layers):
                if j == 0:
                    for h in range(len(self.Network.Layers[j].Neurons)):
                        self.Network.Layers[j].Neurons[h].Value = Data[i][h]
                    continue
    ...
    def TestModel(self):
        ...
        for i in range(len(self.TestingData)):
            for j in range(self.Layers):
                if j == 0:
                    for h in range(len(self.Network.Layers[j].Neurons)):
                        self.Network.Layers[j].Neurons[h].Value = self.TestingData[i][h]
                    continue
                for h in range(len(self.Network.Layers[j].Neurons)):
                    V = 0.0
                    for x in range(len(self.Network.Layers[j - 1].Neurons)):
    ...
    def TestNewSample(self, NewSample):
        if self.BiasUse:
            NewSample.insert(0, 1)
        for j in range(self.Layers):
            if j == 0:
                for h in range(len(self.Network.Layers[j].Neurons)):
                    self.Network.Layers[j].Neurons[h].Value = NewSample[h]
                continue
